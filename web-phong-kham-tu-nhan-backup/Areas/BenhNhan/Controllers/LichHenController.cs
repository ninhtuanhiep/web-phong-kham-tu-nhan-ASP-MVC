...
namespace web_phong_kham_tu_nhan.Areas.BenhNhan.Controllers
{
    ...
    public class LichHenController : Controller
    {
        ...
        ////    {
        ////        ModelState.AddModelError("", "Bác sĩ hiện không làm việc!");
        ////        ViewBag.ChuyenKhoas = new SelectList(_context.Specialties, "Id", "Name");
        ////        return View(model);
        ////    }

        ////    // Validate giờ nếu là ngày hôm nay
        ////    if (model.AppointmentDate.Date == DateTime.Today)
        ////    {
        ////        var validationError = ValidateTimeSlot(model.TimeSlot);
        ...
        public IActionResult GetBookedSlots(int bacSiId, string date)
        {
            if (!DateTime.TryParse(date, out DateTime ngay))
                return Json(new { booked = new List<string>(), past = new List<string>() });
        ...
        //    // Chỉ cho đổi khi trạng thái là Đã xác nhận (1) hoặc Chờ xác nhận (0)
        //    if (lich.TrangThai != 1 && lich.TrangThai != 0)
        //    {
        //        TempData["Error"] = "Lịch hẹn này không thể đổi lịch.";
        //        return RedirectToAction("MyAppointment");
        //    }
        ...
        //    {
        //        TempData["Error"] = "Không thể chọn ngày trong quá khứ.";
        //        return RedirectToAction("Reschedule", new { id });
        //    }

        //    // Validate giờ nếu là hôm nay
        //    if (ngayMoi.Date == DateTime.Today)
        //    {
        ...
        //    lich.TrangThai = 4; // Chờ xác nhận lại

        //    _context.SaveChanges();
        //    TempData["Success"] = "Đã gửi yêu cầu đổi lịch. Vui lòng chờ bác sĩ xác nhận.";
        //    return RedirectToAction("MyAppointment");
        //}
        [HttpPost]
        [ValidateAntiForgeryToken]
